...
extension UIColor {
  public static func convert(from value: Any?) -> Self? {
    // Handle String input (named colors or hex strings)
    ...
    if let opaqueValue = value as? [String: Any] {
      ...
      if let appearances = opaqueValue["dynamic"] as? [String: Any],
         let lightColor = UIColor.convert(from: appearances["light"]) as? Self,
         let darkColor = UIColor.convert(from: appearances["dark"]) as? Self {
        let highContrastLightColor = UIColor.convert(from: appearances["highContrastLight"]) as? Self
        let highContrastDarkColor = UIColor.convert(from: appearances["highContrastDark"]) as? Self

        #if os(iOS) || os(tvOS)
        let color = UIColor { (traitCollection: UITraitCollection) -> UIColor in
          if traitCollection.userInterfaceStyle == .dark {
            if traitCollection.accessibilityContrast == .high, let highContrastDarkColor = highContrastDarkColor {
              return highContrastDarkColor
            }
            return darkColor
          }

          if traitCollection.accessibilityContrast == .high, let highContrastLightColor = highContrastLightColor {
...
extension CGColor {
  public static func convert(from value: Any?) -> Self? {
    if let uiColor = UIColor.convert(from: value) {
      return uiColor.cgColor as? Self
    }
    return nil
  }
}
